...
def archive_request__OLD(request,waveform_clients,sds_path,db_path):
    ...
    # Now loop through traces *** huge problem here for highly fragmented data. Need to do by day TODO TODO
    for tr in st:
...
def archive_request(request, waveform_clients, sds_path, db_path):
    ...
    traces_by_day = defaultdict(obspy.Stream) # new import requirement: from collections import defaultdict
        ...
        if len(pruned_requests) > 0:
            ...
            for ele in credentials:
                ...
                try:
                    ...
                    print("Issue creating client: %s %s via %s:%s" % \
                          (config['WAVEFORM']['client'],ele[0],uname,pw))
                    continue
                waveform_clients.update({ele[0]:new_client})

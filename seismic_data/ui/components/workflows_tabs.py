...
class EventBasedWorkflow:
    ...
    def render(self):
        tab1, tab2, tab3 = st.tabs(["📄 Event", "🌍 Station", "Waveform"])
        with tab1:
        ...
        with tab2:            
            ...
            if len(self.event_components.settings.event.selected_catalogs)>0 :                    
                ...
                if 'station_rendered' not in st.session_state:
                    st.session_state.station_rendered = True
                    print("XXXXX")
                    st.rerun()
            else:
                if 'station_rendered' in st.session_state:
                    st.session_state.pop('station_rendered')
